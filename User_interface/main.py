...
def read_file(filepath, graph_type, canvas, fig, highlight_interval=None):
    ...
    for line in data_lines:
        ...
        if(len(file_values) == 8 or len(file_values) == 3):
            values = list(map(float, file_values[:-1]))
        else:
            values = list(map(float, file_values))
